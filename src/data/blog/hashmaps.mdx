...
I'm going to show you how I took an academic paper, understood it deeply enough to implement it, and used AI as a learning collaborator rather than a code generator. By the end, you'll be able to read the screenshot below and know exactly what it means. And if you stick around: I built something that beats SwissTable at 99% load.
...
In 1985, Andrew Yao<Footnote id="2" /> claimed that for a certain class of hash tables, you couldn't do better than uniform probing (randomly checking slots until you find what you're looking for). The fuller the table, the worse it gets, exponentially so. For four decades, nobody seriously challenged this.
...
There's a function φ that takes an array index and probe number, and tells you what order to check slots. Early probes in small arrays get priority.
...
But I wanted to see how it compared to production code. Benchmarking against Zig's `std.HashMap` (which uses SwissTable under the hood) sent me down more rabbit holes. The final implementation combines:

- **SIMD 16-slot buckets** that scan fingerprints in parallel
- **Batch insertion** from the paper's φ priority function
- **Bitmask tricks** (`@ctz`) to find empty slots without branching

The hybrid beats SwissTable at 99% load—1.5x faster inserts, and faster lookups at both small and large scales. Next week I'll do a deep dive on the implementation: why there's a "cache locality valley" in the middle, how the batch algorithm actually works, and what I learned about SIMD along the way.

For now, [the code is on GitHub](https://github.com/joshuaisaact/elastic-hash).
...
---
*The paper: [Elastic Hashing](https://arxiv.org/pdf/2501.02305). The code: [GitHub](https://github.com/joshuaisaact/elastic-hash). Next week: the deep dive on the hybrid implementation that beats SwissTable.*
