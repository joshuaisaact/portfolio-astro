...
1. **The Monolith** - One prompt does everything. Simple to start, nightmare to maintain. Where I began.
2. **Tool-Calling Agent** - Single agent that calls different functions. What OpenAI's Assistants API demonstrates. Good for simpler use cases.
3. **Orchestrated Specialists** - Separate agents for separate concerns. Intent recognition split from response generation. What Maya became.
4. **Agent Frameworks** - LangChain, LangGraph. Agents delegating to agents. Powerful but often overkill.
