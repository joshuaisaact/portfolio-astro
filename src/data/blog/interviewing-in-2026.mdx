...
The signal: the first version holds thousands of hydrated ORM objects in memory during a 2-second external call. Under load, those objects survive garbage collection, get promoted to old gen, and trigger stop-the-world pauses. P95 explodes.
...
**Design a registration system for sporting events. Walk me through how a user signs up.**

Candidate sketches: User → API → Database → Confirmation Email.

Interviewer pushes: "The email provider has a 30% rate limit. What happens during a flash sale when 10,000 people register in a minute?"

*"We probably need a queue between registration and email. Actually, should we even send confirmation synchronously? Maybe we write to the database, return success, and process emails async. What's the business requirement if emails are delayed by 30 seconds?"*

The signal: nobody designs a system perfectly in their head. Real engineering is iterative. You propose, get pushback, revise. This tests the same depth as a whiteboard interview but rewards collaboration instead of performance.
