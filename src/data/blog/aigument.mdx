...
I also wanted to see how far I could push Next.js without a separate backend, try my first edge database with [Neon](https://neon.tech/), and go deep on the [Vercel AI SDK](https://ai-sdk.dev/).
...
The prompt above went through dozens of iterations. Multi-turn, persona-driven prompts are hard to get right. The LLM drifts off-persona, repeats itself, or goes limp after a few rounds. Each personality needed different guardrails.

Neon was the easiest part. Serverless PostgreSQL, plugged into Next.js Server Actions, done. My first edge database and it took less time to set up than the personality system.
