...
The checkout demo models one flow: three customers sharing two items. But in production, you don't have one net running all customers. You have thousands of concurrent orders, each with their own state.

There are two approaches. **Coloured Petri nets** put data on the tokens. Instead of `paymentPending: 2` you have `paymentPending: [{ orderId: 47 }, { orderId: 52 }]`. Transitions get guard functions that pattern-match across places: "take the token from `paymentPending` where `orderId` matches the one from `inventoryReserved`." This is powerful, but `canFire` goes from a simple count check to a combinatorial search over token bindings. Worse, every distinct combination of data values in every place is a different state. Three customers with IDs already produces a much larger state space than three anonymous tokens. Exhaustive analysis becomes infeasible fast.

**One net per instance** is the simpler answer. Each order gets its own net with anonymous tokens. You keep the simple engine. You keep exhaustive analysis. You prove each individual workflow is sound. Cross-instance concerns like inventory contention get handled where they already live: a database row lock or an atomic decrement before `beginCheckout` fires. The net proves the workflow is correct. The database proves the inventory is consistent. And since a marking is just `Record<Place, number>`, it serialises to a JSON column on the order row. Read the marking, call `fire()`, write it back, all in one transaction.
