...
**One net per instance** is the simpler answer. Each order gets its own net with anonymous tokens. You keep the simple engine. You keep exhaustive analysis. You prove each individual workflow is sound. Cross-instance concerns like inventory contention get handled where they already live: in the database, with the concurrency primitives you already trust. The analyser proves the workflow topology is correct. The database proves the shared resources are consistent. Each tool does what it's good at.
