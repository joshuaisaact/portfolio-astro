...
function initializeTiers(): TierSlots {
  return TIERS.map(size =>
    Array(size).fill(null).map(() => ({ key: null, probes: 0 }))
  );
}
...
function getSlotColor(probes: number, isEmpty: boolean): string {
  ...
  if (probes <= 2) return "bg-emerald-400 dark:bg-emerald-500";
  if (probes <= 4) return "bg-yellow-400 dark:bg-yellow-500";
  if (probes <= 6) return "bg-orange-400 dark:bg-orange-500";
...
const ElasticHashingDemo: React.FC = () => {
  ...
  const [lastInsert, setLastInsert] = useState<{ key: string; tier: number; slot: number; probes: number } | null>(null);
  ...
  const insert = useCallback(() => {
    ...
    for (let offset = 0; offset < maxTierSize && !inserted; offset++) {
      for (let tierIdx = 0; tierIdx < TIERS.length && !inserted; tierIdx++) {
        ...
        if (newTiers[tierIdx][slotIdx].key === null) {
          newTiers[tierIdx][slotIdx] = { key, probes };
          inserted = true;
          insertTier = tierIdx;
          insertSlot = slotIdx;
        }
    ...
    if (inserted) {
      setTiers(newTiers);
      setLastInsert({ key, tier: insertTier, slot: insertSlot, probes });
      ...
      for (let offset = 0; offset < maxTierSize && !inserted; offset++) {
        for (let tierIdx = 0; tierIdx < TIERS.length && !inserted; tierIdx++) {
          ...
          if (currentTiers[tierIdx][slotIdx].key === null) {
            currentTiers[tierIdx][slotIdx] = { key, probes };
            inserted = true;
          }
        }
      }
  ...
  return (
    <div className="my-8 p-4 sm:p-6 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Same 50 slots, but spread across multiple tiers
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div className={`text-2xl font-bold ${maxProbes > 10 ? 'text-red-500' : 'text-emerald-500'}`}>
            {maxProbes}<span className="text-base text-gray-400 dark:text-gray-500">/{TOTAL_SLOTS}</span>
          </div>
              ...
              {tier.map((slot, slotIdx) => {
                ...
                return (
                  <div
                    ...
                    `}
                    title={slot.key ? `${slot.key} (${slot.probes} probes)` : 'Empty'}
                  />
                );
              })}
            </div>
      ...
      <div className="flex flex-wrap justify-center gap-3 text-xs mb-4">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-gray-600 dark:text-gray-400">1-2 probes</span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-gray-600 dark:text-gray-400">5-6 probes</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded bg-red-500"></div>
          <span className="text-gray-600 dark:text-gray-400">7+ probes</span>
        </div>
      </div>
      ...
      {lastInsert && (
        <div className="text-center text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
          ...
          {' '}in <span className={lastInsert.probes > 6 ? 'text-orange-500' : 'text-emerald-500'}>{lastInsert.probes} probes</span>
        </div>
      )}
    </div>
  );
